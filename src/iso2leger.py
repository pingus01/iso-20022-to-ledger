...
Pfad = pathlib.Path(sys.argv[2]).parent.absolute()
Datei = pathlib.Path(sys.argv[2]).absolute()
Dateiname = pathlib.Path(sys.argv[2]).stem
FileOutput = str(Pfad)+'\\'+str(Dateiname)+'.journal'
# print(FileOutput)
...
#                print(Inhaber)
        ...
        for Entry in Statement.findall(namespace+"Ntry"):
            ...
            print(BookDate+' * '+AddtlNtryInf.replace("\n", " ") +
                  ' ' + UserTxDetails)
            FileOutputStream.write(
                BookDate+' * '+AddtlNtryInf.replace("\n", " ")+'\n' + ' ' + UserTxDetails)

            # Amt: Betrag mit Währumg Ccy
            ...
            if CdtDbtInd == 'DBIT':
                print('  '+'assets:Bank:'+Finanzinstitut +
                      ':'+Inhaber, Konto+'  -' + Amt, Currency)
                FileOutputStream.write('  '+'assets:Bank:'+Finanzinstitut +
                                       ':'+Inhaber+' '+Konto+'  -' + Amt + ' ' + Currency + '\n')
                print('  '+'unknown'+'  ' + Amt, Currency)
                FileOutputStream.write(
                    '  '+'unknown'+'  ' + Amt + ' ' + Currency + '\n')
